    ...
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <aside className="w-full lg:w-64 shrink-0">
        <div className="sticky top-20 space-y-4">
          ...
          {/* Header Reference */}
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-3.5 space-y-1">
          ...
          <div className="space-y-1">
            <div className="px-3 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-wider flex items-center gap-1.5">
            ...
            {systemSections.map(section => (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-left text-xs font-medium transition cursor-pointer ${
                  activeSection === section.id
                    ? 'bg-emerald-950/70 text-emerald-300 border border-emerald-800/60 shadow-xs'
                    : 'text-gray-400 hover:bg-gray-900 hover:text-gray-200 border border-transparent'
                }`}
              >
                <span className={activeSection === section.id ? 'text-emerald-400' : 'text-gray-500'}>
                  {section.icon}
                </span>
          ...
          <div className="space-y-1 pt-2 border-t border-gray-800/80">
            ...
            {apiSections.map(section => (
              ...
              >
                <span className={activeSection === section.id ? 'text-sky-400' : 'text-gray-500'}>
                  {section.icon}
                </span>
                <span className="truncate">{section.label}</span>
                {section.endpoints && (
                  <span className="ml-auto text-[10px] px-1.5 py-0.2 rounded-md bg-gray-950 text-gray-400 font-mono border border-gray-800">
      ...
      <main className="flex-1 min-w-0 space-y-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 space-y-2">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <h2 className="text-lg sm:text-xl font-bold text-gray-100 mt-1">{active.label}</h2>
            </div>
          </div>
        ...
        {active.category === 'system' && active.guideBlocks && (
          <div className="space-y-4">
            {active.guideBlocks.map((block, idx) => (
              <div key={idx} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 sm:p-6 space-y-3.5 shadow-xs">
                ...
                {block.items && (
                  <div className="grid grid-cols-1 gap-2 pt-1">
                    {block.items.map((it, i) => (
                      <div key={i} className="p-3 bg-gray-950 border border-gray-800 rounded-xl space-y-1">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                          <span className="text-xs font-semibold text-emerald-400">{it.label}</span>
                          <span className="text-xs font-mono text-gray-200">{it.value}</span>
                        </div>
                        {it.desc && (
                          <p className="text-[11px] text-gray-400 pt-0.5">{it.desc}</p>
                        )}
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-1 pt-1">
                    <div className="flex items-center justify-between text-[11px] text-gray-400 font-mono px-1">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 text-emerald-400 hover:text-emerald-300 transition cursor-pointer font-sans"
                      >
        ...
        {active.category === 'api' && active.endpoints && (
          <div className="space-y-4">
            {active.endpoints.map((ep, idx) => (
              <div key={ep.path + idx} className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-xs">
                {/* Endpoint Header */}
                <div className="px-5 py-3.5 border-b border-gray-800 bg-gray-950/60 flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className={`px-2.5 py-1 rounded-lg border text-[11px] font-mono font-bold shadow-xs ${methodColor[ep.method]}`}>
                      {ep.method}
                    </span>
                    <code className="text-xs sm:text-sm text-gray-200 font-mono font-semibold">{ep.path}</code>
                  </div>
                  <span className="text-[10px] px-2.5 py-0.5 rounded-full bg-gray-950 border border-gray-800 text-gray-400 font-mono">
                    {ep.auth}
                  </span>
                </div>
                ...
                <div className="px-5 py-4 space-y-3.5">
                  <p className="text-xs sm:text-sm text-gray-300 leading-relaxed">{ep.desc}</p>
                  ...
                  {ep.body && (
                    <div className="space-y-1">
                      ...
                      <pre className="bg-gray-950 border border-gray-800 rounded-xl p-3 text-xs text-sky-300 font-mono overflow-x-auto">
                        {ep.body}
                      </pre>
                    </div>
                  )}
                  ...
                  <div className="space-y-1">
                    ...
                    <pre className="bg-[#0d1117] border border-gray-800 rounded-xl p-3.5 text-xs text-gray-200 font-mono overflow-x-auto whitespace-pre-wrap leading-relaxed">
                      {ep.curl}
                    </pre>
                  </div>
                  ...
                  {ep.response && (
                    <div className="space-y-1">
                      ...
                      <pre className="bg-gray-950 border border-gray-800 rounded-xl p-3 text-xs text-emerald-300 font-mono overflow-x-auto">
                        {ep.response}
                      </pre>
                    </div>
                  )}
