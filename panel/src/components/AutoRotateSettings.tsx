...
export const AutoRotateSettings: React.FC<AutoRotateSettingsProps> = ({ showToast }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      {/* Top Banner Status */}
      <div className="bg-gray-900/80 border border-gray-800 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-3.5">
          <div className={`p-3 rounded-2xl border ${enabled ? 'bg-emerald-950/60 border-emerald-800/50 text-emerald-400' : 'bg-gray-800/60 border-gray-700 text-gray-400'}`}>
            <ArrowRightLeft size={22} className={enabled ? 'animate-pulse' : ''} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-base font-bold text-gray-100">Status Session Pooling</h3>
              <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full border ${enabled ? 'bg-emerald-950 text-emerald-400 border-emerald-800/60' : 'bg-gray-800 text-gray-400 border-gray-700'}`}>
        ...
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex items-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white text-xs font-semibold rounded-xl shadow-lg shadow-emerald-950/40 transition cursor-pointer"
        >
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-5 space-y-5">
          <div className="flex items-start justify-between gap-4 border-b border-gray-800/70 pb-4">
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 pt-1">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-gray-950 border border-gray-800 rounded-xl px-3.5 py-2.5 text-xs text-gray-200 focus:outline-none focus:border-emerald-500 font-medium cursor-pointer"
              >
            ...
            <div>
              ...
              <div className="flex items-center justify-between bg-gray-950 border border-gray-800 rounded-xl px-3.5 py-2">
                <div className="pr-2">
                  <span className="text-xs text-gray-200 font-medium block">Pertahankan Nomor Sebelumnya</span>
                  <span className="text-[10px] text-gray-500">Jika penerima pernah chat dengan Nomor A, tetap kirim via Nomor A jika sehat.</span>
                </div>
                <input
                  type="checkbox"
        ...
        <div className="bg-gray-900/60 border border-gray-800 rounded-2xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
            <label className="flex items-start gap-3 bg-gray-950/70 border border-gray-800/80 rounded-xl p-3.5 cursor-pointer hover:border-gray-700 transition">
              <input
                type="checkbox"
                checked={rotateOnLimit}
                onChange={(e) => setRotateOnLimit(e.target.checked)}
                className="mt-0.5 rounded border-gray-700 text-emerald-600 focus:ring-emerald-500 w-4 h-4 cursor-pointer"
              />
              <div className="text-xs">
                <span className="font-semibold text-gray-200 block">Limit Harian Warm-Up</span>
                <span className="text-[11px] text-gray-400 mt-0.5 block">Pindah saat nomor mencapai batas kuota harian anti-ban.</span>
              </div>
            </label>
            ...
            <label className="flex items-start gap-3 bg-gray-950/70 border border-gray-800/80 rounded-xl p-3.5 cursor-pointer hover:border-gray-700 transition">
              ...
              />
              <div className="text-xs">
                <span className="font-semibold text-gray-200 block">Socket Disconnected</span>
                <span className="text-[11px] text-gray-400 mt-0.5 block">Pindah saat nomor tiba-tiba terputus atau connecting.</span>
              </div>
            </label>
            ...
            <label className="flex items-start gap-3 bg-gray-950/70 border border-gray-800/80 rounded-xl p-3.5 cursor-pointer hover:border-gray-700 transition">
              ...
              />
              <div className="text-xs">
                <span className="font-semibold text-gray-200 block">Timelock 463</span>
                <span className="text-[11px] text-gray-400 mt-0.5 block">Pindah jika nomor terkena isolasi kirim ke nomor asing.</span>
              </div>
            </label>
          </div>
        </div>
            ...
            <table className="w-full text-left text-xs min-w-[640px]">
              ...
              <tbody className="divide-y divide-gray-800/60 bg-gray-900/40">
                {roster.map((s: any) => {
                  ...
                  return (
                    <tr key={s.id} className={`transition hover:bg-gray-800/40 ${isIncluded ? '' : 'opacity-50'}`}>
                      ...
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-semibold border ${
                          ...
                          <span className={`w-1.5 h-1.5 rounded-full ${s.status === 'connected' ? 'bg-emerald-400' : 'bg-red-400'}`} />
                          {s.status}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        {warmup ? (
                          <div className="space-y-1 w-32">
                            <div className="flex justify-between text-[10px]">
                              <span className={limitReached ? 'text-red-400 font-bold' : 'text-gray-300'}>
                                {warmup.todaySent} / {warmup.todayLimit} pesan
                              </span>
                              <span className="text-gray-500 font-mono">H-{warmup.day}</span>
                            </div>
                            <div className="w-full bg-gray-800 h-1.5 rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all ${limitReached ? 'bg-red-500' : 'bg-emerald-500'}`}
                      ...
                      <td className="px-4 py-3">
                        {s.timelockActive ? (
                          <span className="text-[10px] text-amber-400 bg-amber-950/60 px-2 py-0.5 rounded border border-amber-800/50 flex items-center gap-1 w-fit">
                            <ShieldAlert size={12} /> Restricted
                          </span>
                        ) : (
                          <span className="text-[10px] text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-800/50 flex items-center gap-1 w-fit">
                            <Check size={12} /> Clear
                          </span>
                        )}
                      </td>
